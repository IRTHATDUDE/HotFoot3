    ...
    public boolean onCommand(CommandSender sender, Command command, String s, String[] args) {
        if(command.getName().equalsIgnoreCase("hfpos1")){
            if(!(sender instanceof Player)){
                sender.sendMessage(ChatColor.RED + "You cannot execute this command on console");
                return false;
            }
            if(args.length==0){
                sender.sendMessage(ChatColor.RED + "You need to have a name for the position you are marking");
                return false;
            }
            ...
            for(int i = 0; i<4;i++){
                if(args[0].equalsIgnoreCase(same.get(i).getName())){
                    ...
                    player.sendMessage("You have selected " + x +","+y+","+z+" for Position 1 Arena " + same.get(i).getName());
                    same.get(i).setLocation1(loc);
                }
            }
        }
        return false;
